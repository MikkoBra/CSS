...
class ClusterSizePlot:
    ...
    def plot_power_law(self, bin_centers, normalized_hist, ax, label_power_law):
        try:
            # Find the point where the data starts increasing
            increasing_idx = np.argmax(normalized_hist[1:] > normalized_hist[:-1]) + 1  # Find first increase

            # Slice the data to include only the part where the histogram is still decreasing
            truncated_bin_centers = bin_centers[:increasing_idx]
            truncated_normalized_hist = normalized_hist[:increasing_idx]

            # Log-bin centers and log-normalized histogram
            log_bin_centers = np.log10(truncated_bin_centers)
            log_normalized_hist = np.log10(truncated_normalized_hist)

            # Define a linear function for log-log fitting (log(P) = log(a) + b * log(x))
            def log_power_law(log_x, log_a, b):
                return log_a + b * log_x

            # Perform curve fitting in log space (only on the truncated data)
            popt, pcov = curve_fit(log_power_law, log_bin_centers, log_normalized_hist, maxfev=10000)
            log_a, b = popt
            ...
            fitted_curve = a * truncated_bin_centers ** b
            # Plot the fitted curve
            label = ''
            if label_power_law:
                label = 'Power Law Distribution'
            ax.plot(truncated_bin_centers, fitted_curve, 'r--', alpha=0.6, lw=1.5, label=label)
            print(normalized_hist, fitted_curve)
            self.ks_test(normalized_hist, fitted_curve)
        except ValueError:
            print("Curve fitting failed.")
    ...
    def plot_single_cluster_size(self, results_per_system_size, system_size, title, file_name, show_plot=False):
        ...
        self.plot_single_cluster_size_probability_vs_cluster_size_with_power_law(ax, True)
