...
for size, color, ax in zip(sizes, colors, axes):
    ...
    ax.axvline(critical_density, color='k', linestyle='--', label=f'?: {critical_density:.2f}')
...
display(pd.DataFrame({'Size': sizes, 'Critical Density': [axes[i].lines[-1].get_xdata()[0] for i in range(6)]}))
